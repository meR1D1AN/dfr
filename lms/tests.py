...
class LessonTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.moderator = User.objects.create(email='moderator@example.com', password='modpassword')
    ...
    def test_update_lesson(self):
        ...
        response = self.client.put(reverse('lms:lesson_detail', kwargs={'pk': self.lesson.pk}), updated_data,
                                   format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
# #         self.client.force_authenticate(user=self.user)
# #         response = self.client.post(reverse('subscription'), {'course_id': self.course.id}, format='json')
# #         print(response.json())
# #         self.assertEqual(response.status_code, status.HTTP_201_CREATED)
